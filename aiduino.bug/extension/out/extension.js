...
// ===== TOKEN MANAGEMENT =====
function setupTokenManagerEvents() {
    // Listen to token updates for status bar
    tokenManager.onTokenUpdate(event => {
        if (event.type === 'usage_recorded' || event.type === 'initialized') {
            updateStatusBar();
        }
        
        if (event.type === 'save_error') {
            console.warn('Token save failed:', event.error);
        }
    });
    
    // Listen to quota warnings
    tokenManager.onQuotaWarning(event => {
        const providerName = minimalModelManager.providers[event.modelId]?.name || event.modelId;
        const percentage = Math.round(event.ratio * 100);
        
        if (event.level === 'critical') {
            vscode.window.showErrorMessage(
                `⚠️ ${providerName}: ${percentage}% des Kontingents erreicht!`,
                'Modell wechseln', 'Ignorieren'
            ).then(choice => {
                if (choice === 'Modell wechseln') {
                    switchModel();
                }
            });
    ...
    // Listen to daily resets
    tokenManager.onDailyReset(event => {
        updateStatusBar();
        vscode.window.showInformationMessage(
            `📊 Token-Statistik für ${event.newDate} zurückgesetzt`
        );
    });
}
...
function activate(context) {
    ...
    // Initialize TokenManager BEFORE other components
    tokenManager = new TokenManager(minimalModelManager.providers);

     // Setup TokenManager event listeners
    ...
    if (deps.uiTools && deps.uiTools.shouldShowWelcome(deps)) {
    setTimeout(async () => {
        await deps.uiTools.showWelcomeMessage(deps);
    }, 1000);
}
}
 ...
 * @param {vscode.ExtensionContext} context - VS Code extension context
 */
// In extension.js - registerCommands():
function registerCommands(context) {
    commandRegistry = new CommandRegistry();
    
    const commandDeps = {
        getDependencies,  // Das reicht - enthält bereits alles
        minimalModelManager  // Nur für das Debug-Command nötig
    };
...
function updateStatusBar() {
    ...
    if (hasApiKey && tokenManager) {
        const usage = tokenManager.getModelUsage(currentModel);
        const costDisplay = usage.cost > 0 ? ` ($${usage.cost.toFixed(3)})` : '';
        
        statusBarItem.text = `${providerInfo.icon} AI.duino${costDisplay}`;
        statusBarItem.tooltip = 
            `${providerInfo.name}\n` +
            `Tokens: ${usage.input + usage.output}${costDisplay}\n` +
